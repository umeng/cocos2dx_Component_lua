...
int lua_umeng_mobclickcpp_exchange(lua_State* L)
{
    int paranum = 0;
    paranum = lua_gettop(L);
    if(paranum == 5){
        ...
        const char* order;
        string orderId;
        ok &= luaval_to_std_string(L,1,&orderId,"umeng_exchange");
        order = orderId.c_str();
                
        double currencyAmount;
        ok &= luaval_to_number(L,2,&currencyAmount,"umeng_exchange");
        
        const char* cTy;
        string cTy_s;
        ok &= luaval_to_std_string(L,3,&cTy_s,"umeng_exchange");
        cTy = cTy_s.c_str();
        
        double virtualAmount;
        ok &= luaval_to_number(L,4,&virtualAmount,"umeng_exchange");
        
        int channel;
        ok &= luaval_to_int32(L,5,&channel,"umeng_exchange");
        if(ok){
            umeng::MobClickCpp::exchange(order,currencyAmount,cTy,virtualAmount,channel);
            return UMENG_SUCCEED;
        }
    }
    return UMENG_FAILED;
...
int lua_umeng_dplus_unregisterSuperProperty(lua_State* L)
{
    int paranum = 0;
    paranum = lua_gettop(L);
    if (paranum == 1)
...
int lua_umeng_dplus_setFirstLaunchEvent(lua_State* L)
{
    int paranum = 0;
    paranum = lua_gettop(L);
    if (paranum == 1)
